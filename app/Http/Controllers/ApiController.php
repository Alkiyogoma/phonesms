    ...
    public function pushPhoneSMS()
    {
        ...
        if (count($verify) > 0) {
            foreach ($verify as $value) {
                    ...
                    $link = $value->name;
                    $messages = DB::select("select a.contact as phone,  a.message_id as id, a.content as body from message a where user_id = '".$value->id."' AND sms_status != 1 order by created_at limit 30");
